...
**Execution order:** A (time-sensitive) → B → C → D → E → F → G → H. Each task: implement (fresh subagent, full context in prompt) → spec review → quality review → fixes. Final: full verification (root tests, `pnpm -r typecheck`, `pnpm --filter @bcc/web build`), update spec checkboxes, report.
